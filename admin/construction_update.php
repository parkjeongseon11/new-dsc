<?php
	...
	if($id){
		sql_query("update `rutilo_construction` set `title`='{$title}',`content`='{$content}',`videolink`='{$videolink}',`etc`='{$etc}', `step`='{$step}' {$photo_sql} where `id`='{$id}';");
	}else{
		sql_query("insert into `rutilo_construction` (`title`,`content`,`videolink`,`photo`,`etc`,`step`) values('{$title}','{$content}','{$videolink}','{$photo}','{$etc}', '{$step}');");
	}
