				...
				<table>
					...
					<?php
						for($i=0;$i<count($list);$i++){
                            ...
                            switch($list[$i]['return']){
                                case "3":$status="반품요청";break;
                                case "2":$status="수거완료";break;                                
                            }
							$sql = "select * from `rutilo_delivery` where cart_id = '{$list[$i][cart_id]}'";
							$deli = sql_fetch($sql);
							switch($deli["status"]){
					...
					?>
						...
						<tr>
							...
							<td onclick="location.href='<?php echo G5_URL."/admin/reserve_view.php?id=".$list[$i]['id']."&page=".$page."&s=".$s."&m=".$m."&b".$b; ?>'"><?php echo hyphen_hp_number($list[$i]['mb_phone']); ?></td>
                           	<td onclick="location.href='<?php echo G5_URL."/admin/reserve_view.php?id=".$list[$i]['id']."&page=".$page."&s=".$s."&m=".$m."&b".$b; ?>'"><?php echo  $deli['mb_addr']; ?></td>					
							<td onclick="location.href='<?php echo G5_URL."/admin/reserve_view.php?id=".$list[$i]['id']."&page=".$page."&s=".$s."&m=".$m."&b".$b; ?>'"><?php echo $status; ?></td>
							<td>
								<a href="<?php echo G5_URL."/admin/reserve_write.php?id=".$list[$i]['id']."&page=".$page; ?>" class="btn01 btn" style="padding:5px 10px;">수정</a> <a href="<?php echo G5_URL."/admin/reserve_delete.php?id=".$list[$i]['id']."&page=".$page; ?>" class="btn01 btn" style="padding:5px 10px;">삭제 </a>
								<?php if($status=="결제완료" || $status=="배송중"){?>
								 <a href="<?php echo G5_URL."/admin/reserve_delivery.php?id=".$deli['id']."&page=".$page; ?>" class="btn01 btn" style="padding:5px 10px;"><?php if($status=="결제완료"){echo "배송정보입력";}else if($status=="배송중"){echo "배송정보수정";}?></a>
								<?php }?>
							</td>
						</tr>
					<?php	}
