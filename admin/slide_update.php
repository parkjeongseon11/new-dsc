<?php
	...
	if($id){
		sql_query("update `dsc_slide` set `category`='{$category}',`etc`='{$etc}' {$photo_sql} where `id`='{$id}';");
	}else{
		sql_query("insert into `dsc_slide` (`photo`,`category`,`etc`) values('{$photo}','{$category}','{$etc}');");
	}
