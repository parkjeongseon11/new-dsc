<?php
	...
	$total=sql_fetch("select count(*) as cnt from `rutilo_franchisee`");
	if(!$page)
		$page=1;
	$total=$total['cnt'];
	$rows=10;
	...
	<section>
		...
		<article>
				...
				<table>
					...
					?>
						<tr>
							...
							<td><a href="<?php echo G5_URL."/admin/franchisee_delete.php?id=".$list[$i]['id']."&page=".$page; ?>" class="btn01 btn" style="padding:5px 10px;">삭제</a></td>
						</tr>
					<?php
						}
						if(count($list)==0){
			...
			<div class="num_list01">
				<ul>
				<?php if($page!=1){?>
					<li class="prev"><a href="<?php echo G5_URL."/admin/franchisee_list.php?page=".($page-1); ?>">&lt;</a></li>
				<?php } ?>
				<?php for($i=$start_page;$i<=$end_page;$i++){ ?>
					<li class="<?php echo $page==$i?"active":""; ?>"><a href="<?php echo G5_URL."/admin/franchisee_list.php?page=".$i; ?>"><?php echo $i; ?></a></li>
				<?php } ?>
				<?php if($page<$total_page){?>
					<li class="next"><a href="<?php echo G5_URL."/admin/franchisee_list.php?page=".($page+1); ?>">&gt;</a></li>
				<?php } ?>
				</ul>
			</div>
			<?php
